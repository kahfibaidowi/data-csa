...
use App\Repository\CurahHujanNormalRepo;
use App\Models\CurahHujanModel;
use App\Models\CurahHujanNormalModel;

class CurahHujanNormalController extends Controller
{

    public function copy_from_curah_hujan(Request $request)
    {
        ...
        DB::transaction(function() use($req, &$curah_hujan){
            $ch=CurahHujanModel::where("tahun", "2023")->get()->toArray();

            foreach($ch as $val){
                CurahHujanNormalModel::create([
    ...
    public function upsert(Request $request)
    {
        ...
        $curah_hujan=(object)[];
        DB::transaction(function() use($req, &$curah_hujan){
            $q=CurahHujanNormalModel::lockForUpdate()
                ->first();

            $update=CurahHujanNormalModel::updateOrCreate(
                [
    ...
    public function upsert_multiple(Request $request)
    {
        ...
        DB::transaction(function() use($req){
            ...
            foreach($req['data'] as $val){
                CurahHujanNormalModel::updateOrCreate(
                    [
                        'id_region' =>$val['id_region'],
                        'bulan'     =>$val['bulan'],
                        'input_ke'  =>$val['input_ke']
    ...
    public function gets_treeview(Request $request)
    {
        ...
        //SUCCESS
        $curah_hujan=CurahHujanNormalRepo::gets_curah_hujan_normal_treeview($req);
